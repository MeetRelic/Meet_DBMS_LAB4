--  i created a new schema in local db name- test_dbms_lab4 
use test_dbms_lab4;
...
-- in question the date is wrong here month cant be 00 for orderId 109 , so adding 09 as month-- 
...
as t1  group by t1.ord_id) as t2 group by t2.cus_gender;
...
-- Attached stored procedured sql file seperately 
...
call proc();
-- Delimiter is ; which we have used in our query we can customly define Delimeter and use it 
-- solution is atatched below and also in seperate storeproc.sql file 
...
--                 supplier
--             WHERE
--                 final.supp_id = supplier.supp_id
--         ) AS report;
-- END
